...
| append_only                             | Boolean   |              | (Default: False) When `append_only` is set to true records will only ever be appended to the destination table. Even when a primary key is detected updates will not be attempted. It is also worth noting that when this setting is enabled `hard_delete` will do nothing, `add_metadata_columns` will automatically be enabled, and no record debounce will occur (eg. every record change from a `LOG_BASED` stream will be captured). |
